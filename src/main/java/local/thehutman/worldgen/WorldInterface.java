...
public class WorldInterface {
	...
	public WorldInterface(Player player, String nameClass) {
		...
		try {
			...
			// Field fWorldProvider =
			// clObjWorldServer.getField("worldProvider");
			// Object oWorldProvider = fWorldProvider.get(oCraftWorldHandle);

			// Method mGetChunkProvider =
			// clObjWorldProvider.getMethod("getChunkGenerator");
			// Object oChunkProvider = mGetChunkProvider.invoke(oWorldProvider);

			// Field randField = getChunkProvider(player);
			oRandom = new Random();// (Random) randField.get(oChunkProvider);
	 ...
	 */
	// private Field getChunkProvider(Player player) throws
