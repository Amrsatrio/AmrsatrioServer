...
class GenType6 {
	...
	public static void generate(Player player, int radius, String namePerm, String nameDisplay, String nameClass,
			String nameFID, int Offset) throws Exception {
		...
		Constructor<?> cBox = i.clObjStrucBox.getConstructor(int.class, int.class, int.class, int.class, int.class,
				int.class);
		...
		Constructor<?> cBuild = clBuild.getConstructor(clStartPiece, int.class, i.oRandom.getClass(), i.clObjStrucBox,
				int.class);
		...
		if (!r) player.sendMessage(ChatColor.RED + "Unable to generate a " + nameDisplay + " at this location.");
		else {
