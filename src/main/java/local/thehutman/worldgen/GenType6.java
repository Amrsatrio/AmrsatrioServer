...
class GenType6 {
	 ...
	 * @param nameFID     Name of the field ID used to flag rendering of the village
	 *                    (some use a, some b)
	 * @param Offset      Vertical start offset from current land position.
	 */
	public static void generate(Player player, int radius, String namePerm, String nameDisplay, String nameClass, String nameFID, int Offset) throws Exception {
		...
		WorldInterface i = new WorldInterface(player, "WorldGenVillage$WorldGenVillageStart");
		...
		// Get start position
		Method mStartPiece = i.clObjGenerator.getMethod("c");
		@SuppressWarnings("rawtypes") LinkedList lPieces = (LinkedList) mStartPiece.invoke(oGen);

		// Now, create an instance of our custom class
		Class<?> clStartPiece = Class.forName(i.pckMinecraft + ".WorldGenVillagePieces$WorldGenVillageStartPiece");
