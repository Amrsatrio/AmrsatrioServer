...
import net.minecraft.server.v1_12_R1.LootTableInfo.a;
...
public class ServerPlugin extends JavaPlugin implements Listener, PluginMessageListener {
    ...
    public static final String SUPPORTED_NMS_VERSION = "v1_12_R1";
    ...
    //	private static final List<UUID> ALLOWED_TO_KICK_BAN_DEOP = Arrays.asList(UUID.fromString("77a3d6a0-49d5-45eb-bcb8-48dc26303c43"), UUID.fromString("beaa6a95-f065-4b75-883f-894488ec133e"));
    private static final List<String> PASSWORDED_COMMANDS = Arrays.asList("password", /*"anon", */"togglebantnt", "togglechatcaps", "restart", "stop", "toggleverbose", "toggledmgsummary"); // , "ban", "kick", "op", "deop"
...
//	}
    ...
    private IInventory getLootTableInventory(Player player, String s) {
        ...
        LootTable loottable = worldserver.getLootTableRegistry().a(new MinecraftKey(s));
    ...
    public synchronized boolean a(net.minecraft.server.v1_12_R1.World world, Random random, ChunkCoordIntPair chunkcoordintpair, StructureStart structurestart) {
    ...
    public void a(final PlayerCommandPreprocessEvent playercommandpreprocessevent) {
        ...
        //		List<String> commandSpecificPlayers = Arrays.asList("ban", "kick", "op", "deop");
...
//		}
        ...
        if (PASSWORDED_COMMANDS.contains(label)) {
            ...
            makeAnAuth(playercommandpreprocessevent.getPlayer(), astring, () -> Bukkit.dispatchCommand(playercommandpreprocessevent.getPlayer(), s));
        }
    }
...
//		}
    ...
    private IChatBaseComponent getPingedPlayerNames(PingEntry pingentry) {
        ...
        return new ChatMessage(joinNiceString(astring, "or"), pingentry.names.stream().map(GAME_PROFILE_TO_NAME_FUNCTION::apply).collect(Collectors.toList()));
    }
    ...
    private IChatBaseComponent damageSummary(Damage damage, boolean fromMe) throws Exception {
        ...
        if (entitydamageevent instanceof EntityDamageByEntityEvent) {
            ...
            if (damager instanceof LivingEntity) {
                ...
                net.minecraft.server.v1_12_R1.ItemStack usingItem = el.getItemInMainHand();
    ...
    private boolean genStructure(StructureGenerator a, BlockPosition b, Chunk cc, int d) throws Exception {
        Random r = new Random();
        net.minecraft.server.v1_12_R1.World w2 = ((CraftWorld) cc.getWorld()).getHandle();
...
//		} else structurestart = new WorldGenMonument.WorldGenMonumentStart(w2, r, cc.getX(), cc.getZ());
    ...
    private void makeAnAuth(CommandSender a, String[] b, Runnable c, boolean d) {
        ...
        Authenticator authenticator = new Authenticator(a, b, this, d);

        if (authenticator.auth(c)) {
            if (a instanceof Player) {
                authenticatorInstances.put((Player) a, authenticator);
            }
        }
    }
...
//
                ...
                case "nameitem":
                    ...
                    itemmeta.setDisplayName(buildString(astring, 0).replaceAll("&", "\u00a7"));
                ...
                case "loottable":
                    ...
                    IInventory inv = getLootTableInventory(player, astring[0]);
                    if (astring.length == 2 && astring[1].equals("true")) {
                        InventoryUtils.dropEntity(((CraftWorld) player.getWorld()).getHandle(), ((CraftPlayer) player).getHandle(), inv);
                    } else {
                        player.openInventory(new CraftInventory(inv));
                    }
                    return true;
                ...
                case "text":
                    ...
                    return Texter.text(text.toString().trim(), player, false);
...
//					net.minecraft.server.v1_12_R1.World w2 = ((CraftWorld) p.getWorld()).getHandle();
...
//					return true;
                ...
                case "inputphone":
                    ...
                    try {
                        phoneMap.sendInput(PhoneInput.getByName(astring[0].toLowerCase()), player);
...
//			commandmap.register(getName(), new CommandThrowException());
    ...
    public List<String> onTabComplete(CommandSender commandsender, Command command, String s, String[] astring) {
        try {
            switch (command.getName().toLowerCase()) {
                case "loottable":
                    return astring.length == 1 ? CommandAbstract.a(astring, getAvailableLootTables(commandsender)) : astring.length == 2 ? CommandAbstract.a(astring, Arrays.asList("true", "false")) : Collections.emptyList();

                case "inputphone":
                    return astring.length == 1 ? CommandAbstract.a(astring, Arrays.stream(PhoneInput.values()).map(phoneinput -> phoneinput.toString().toLowerCase()).collect(Collectors.toList())) : Collections.emptyList();
            }
        } catch (Throwable e) {
            broke(e);
        }
    ...
    public void a(EntityDamageEvent entitydamageevent) {
        ...
        try {
            Utils.actionBarWithoutReflection((Player) entitydamageevent.getEntity(), damageSummary(damage));
        } catch (Exception e) {
            broke(e);
        }
    }
