...
public class BukkitCompat implements Listener {
	...
	public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
		 ...
		 */
		String[] IgnoreCommands = { "accept", "addmember", "backupworld", "decline", "delmember", "delplayerdata",
				"giveme", "giveto", "goto", "invite", "join", "lastseen", "lookup", "motd", "opme", "players",
				"serverstats", "userstats", "whois", "creative", "survival", "serverversion", "day", "night" };
		for (String S : IgnoreCommands) {
			if (S.equals(Command)) {
				if (Bukkit.getServer().getPluginCommand(S) != null) {
					...
					this.log.info("Command '" + S + "' conflicts with plugin '" + pName
							+ "', McMyAdmin will not receive this command. Command will be sent to '" + pName
							+ "' instead.");
					break;
				}
				if (event.getPlayer().isOp())
					this.log.info(event.getPlayer().getName() + " issued server command: " + Message);
				else this.log.info(event.getPlayer().getName() + " tried command: " + Message);
