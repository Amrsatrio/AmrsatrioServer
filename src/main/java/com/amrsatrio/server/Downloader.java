...
public class Downloader {
	...
	public static Thread download(final File saveDir, final URL url, final long maxSize, final DownloadListener l,
								  final Player p) {
		Thread th = new Thread(new Runnable() {
			public void run() {
				...
				try {
					...
					String fn;
					if (raw != null && raw.indexOf("=") != -1) fn = raw.split("=")[1].replaceAll("\"", "");
					else {
					...
					if (saveFile.exists()) throw new IOException("That file already exists!");
					...
					if (maxSize > 0 && i > maxSize) throw new IOException(
							"Filesize is bigger than maximum allowed (file is " + i + ", limit is " + maxSize + ")");
					int k = 0;
